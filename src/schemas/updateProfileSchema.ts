...
export const updateProfileSchema = (t: LocaleType) => {
  return z.object({
    ...
    city: z.string({ message: t.formErrors.required }).min(4, t.formErrors.required).max(30, t.formErrors.required),
    country: z.string({ message: t.formErrors.required }).min(2, t.formErrors.required).max(30, t.formErrors.required),
